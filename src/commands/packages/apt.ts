...
export default class apt extends Command {
  ...
  pagination(result) {
    ...
    for (const page of result) {
      const user = result.distro_series_link?.split("/")?.reverse()[0] || "no thing";
      const url = encodeURI("https://packages.ubuntu.com/" + user + "/" + result.source_package_name);
      const emoji =
        Math.floor(Math.random() * 2) + 1 == 1 ? "<:ubuntu:844997080883134505>" : "<:debian:844997118901092443>";
      pages.push(
        this.client.util
          .embed()
          .setColor("#0080ff")
          .addField("❯ Version", page.source_package_version || "no version")
          .addField("❯ Maintainer", page.package_maintainer_link.split("/").reverse()[0] || "no Maintainer")
...
function data(q: string) {
  return axiod(`https://api.launchpad.net/1.0/ubuntu/+archive/primary?ws.op=getPublishedSources`, {
    method: "GET",
    params: {
      source_name: q,
    },
