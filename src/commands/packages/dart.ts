...
export default class dart extends Command {
  ...
  async exec(message: NaticoMessage, { dart }: { dart: string }) {
    try {
      ...
      const data = await axiod(`https://pub.dev/api/packages/${pkg.data.packages[0].package}`);
      if (data.status == 404) {
        return message.reply({
          content: "<:no:838017092216946748> Please provide a valid dart package",
        });
      ...
      const embed = this.client.util
        .embed()
        .setColor("#FF0000")
        .setDescription(result.latest.pubspec.description || "No description provided")
        .addField("❯ Homepage", result.latest.pubspec.homepage || "This package doesnt have a homepage")
      ...
      embed.addField("❯ dependencies", depsMessage ?? "this package doesnt have any dependencies");
